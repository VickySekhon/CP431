...
def parallel_merge_algorithm(exp):
    ...
    # send out A and B
    send_segments_to_ranks(comm, A, a_counts, a_displacements, local_A, root=FIRST, tag=TAG_SEND_A)
    send_segments_to_ranks(comm, B, b_counts, b_displacements, local_B, root=FIRST, tag=TAG_SEND_B)
    ...
    C = receive_segments_from_ranks(comm, local_C, c_counts, c_displacements, root=FIRST, tag=TAG_RECEIVE_C)
    t1 = MPI.Wtime()
    if rank == FIRST:
