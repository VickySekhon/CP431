...
def send_segments_to_ranks(
    comm, sendbuf, counts, displacements, recvbuf, root=0, tag=0
):
    rank = comm.Get_rank()
    size = comm.Get_size()
    ...
    if recvbuf.size != count:
        raise ValueError(
            f"Receive buffer size {recvbuf.size} does not match expected count {count} for rank {rank}"
        )

    if rank == root:
        # root copies it's own slice
...
def receive_segments_from_ranks(comm, sendbuf, counts, displacements, root=0, tag=0):
    ...
    if sendbuf.size != expected:
        raise ValueError(
            f"Send buffer size {sendbuf.size} does not match expected count {expected} for rank {rank}"
        )
...
def merge(A, B, out):
    """2-pointers linear time merge algorithm based on notes from class"""
    i = 0
    j = 0
    k = 0
...
def parallel_merge_algorithm(exp):
    ...
    # send out A and B
    send_segments_to_ranks(
        comm, A, a_counts, a_displacements, local_A, root=FIRST, tag=TAG_SEND_A
    )
    send_segments_to_ranks(
    ...
    C = receive_segments_from_ranks(
        comm, local_C, c_counts, c_displacements, root=FIRST, tag=TAG_RECEIVE_C
    )
    t1 = MPI.Wtime()
    if rank == FIRST:
